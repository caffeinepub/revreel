...
export default function ProfileSetupModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const defaultAvatarUrl = `${window.location.origin}/assets/generated/default-avatar.dim_128x128.png`;
      const avatar = ExternalBlob.fromURL(defaultAvatarUrl);
      // Use the selected image data URL, or empty string to fall back to blob avatar
      const finalAvatarUrl = avatarDataUrl || '';
      await createUser.mutateAsync({ username: username.trim(), bio: bio.trim(), avatar, avatarUrl: finalAvatarUrl });
  ...
  return (
    <Dialog open={true}>
      <DialogContent className="bg-card border-border max-w-md mx-auto" onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
